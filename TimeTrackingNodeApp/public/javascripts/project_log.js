...
function checkSave(){
    ...
    */
     ...
     let check = false;
     if((name.trim().length >=1 && student.trim().length>=1 && task.trim().length >=1 && start_time.length >0 && end_time.length >0 && date.length >0 && description.trim().length >= 1)) {
         check = true;
     }
     ...
     if(!check){
       ...
       var modal = document.getElementById("myModal");
        ...
        addRowToProjectLog(name, student, task, date, start_time, end_time, description, status)
     }
  ...
  function addRowToProjectLog(rowToAdd, name, student, task, date, start_time, end_time, description, status) {
    /*
    ...
    */
    ...
    comment.addEventListener ("click", function() {
      ...
      this.parentNode.parentNode.cells[index].innerHTML = msg; // writes the comment in the comment box
      $.post('/project_log', { rowToAdd, msg })
...
function loadLogs(){
  ...
  */
  ...
  let info = JSON.parse(groupProjectlog)
  console.log(info)
  ...
  for (let i = 0; i < info.length; i++){
    ...
    if (details[8] != null){ 
      ...
      addRowToProjectLog((i + 1), details[0], details[1],details[2] , details[3], details[4], 
